...
fs.readdir('./komutlar/', (err, files) => {
    ...
    log(`Yükleniyor Komutlar Sabret ${files.length} `);
    files.forEach(f => {
...
"Yaşanılacak en güzel mevsim sensin.",
  ...
  client.on("message", async message => {
    if(message.channel.id !== (ayarlar.iltifatkanal)) return;
    let rhode = db.get('chatiltifat');
    await db.add("chatiltifat", 1);
    if(rhode >= 100) {
      db.delete("chatiltifat");
      const random = Math.floor(Math.random() * ((rhodeiltifat).length - 1) + 1);
      message.reply(`${(rhodeiltifat)[random]}`);
    };
  });
...
client.on("guildMemberAdd", member => {
    member.setNickname("Kayıtsız"); // SUNUCUYA BİRİ GİRİNCE DEĞİŞCEK İSİM
  });
...
client.on("ready", () => {
    client.channels.cache.get("kanal ıd").join(); // GİRECEĞİ SES KANALININ İDSİ
  })

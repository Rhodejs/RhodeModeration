...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        await message.channel.send(`${member}`, { embed: embed.setAuthor(member.displayName, member.user.avatarURL({ dynamic: true, size: 2048 })).setDescription(`${message.author} seni kendi kanalına çekmek istiyor ?`) }).then(async msg => {
            await msg.react("✅")
